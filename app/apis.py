...
from app.models import User, Workshops, Talks, College, Contests
from app.forms import AddWorkshop, AddContest, AddTalk, EduData, MoreData
from app.more import level_required, get_user_ip
...
api = Api(apis, version='0.0.1', title="Switch API", description="Switch services API listing.", default='General')

afarer = api.namespace('farer', description="Farer management")
aevents = api.namespace('events', description='Events management')
aquiz = api.namespace('quiz', description='SOY Quiz')

@api.route('/college/list/')
class college_list(Resource):
    def get(self):
        colleges = College.query.all()
        colls = College.serialize_list(colleges)
        for coll in colls:
            del coll['caid']
        return jsonify(colls)

@afarer.route('/users/', endpoint='farer/users')
class farer_user(Resource):
    @login_required
    @level_required(8)
    @api.doc('List of all users with complete details - need level 8 access')
    def get(self):
        users = Users.query.all()
        users = Users.serialize_list(users)
        return jsonify(users)

@afarer.route('/user/current/')
class farer_current(Resource):
    @login_required
    @api.doc('Current user data')
    def get(self):
        return jsonify()

# @afarer.route('/login')
# For Vidyut domain only
# class farer_login(Resource):
    

@afarer.route('/user/education/')
class farer_u_edu(Resource):
    ...
    def put(self):
        ...
        flog = FarerLog(uid=current_user.id, action="Submit Form:EduData")
        db.session.add(flog)
...
class farer_u_edu(Resource):
    ...
    def put(self):
        ...
        if form.validate() == False:
            print(form.errors)
            return jsonify(form.errors)
        flog = FarerLog(uid=current_user.id, action="Submit Form:MoreData")
        db.session.add(flog)
        user = User.query.filter_by(id=current_user.id).first()
